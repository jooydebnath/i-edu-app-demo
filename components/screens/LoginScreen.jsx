...
export default function LoginScreen({ onLogin, onGoRegister }) {
  ...
  const submit = async (e) => {
    ...
    setLoading(false); onLogin({ mobile });
  };
  ...
  return (
    <div className="animate-fade-in flex flex-col px-6 pt-10 pb-6 min-h-full">
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-extrabold text-brand-800 mt-4">Welcome Back!</h1>
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-2xl px-4 py-3 shadow-card focus-within:ring-2 focus-within:ring-brand-300 focus-within:border-brand-400 transition-all">
            ...
            <input type={showPw ? 'text' : 'password'} value={password} onChange={(e) => { setPassword(e.target.value); setErr(''); }} placeholder="Enter password" className="flex-1 bg-transparent outline-none text-sm text-slate-900 placeholder-slate-400" />
      ...
      <p className="text-center text-xs text-slate-500 mt-6">
        ...
        <button onClick={onGoRegister} className="font-bold text-brand-700 hover:text-brand-900 transition-colors">Register</button>
      </p>
    </div>
  );
}
